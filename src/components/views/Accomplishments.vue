...
<template>
	<PageContainerVue>
		<header>
			<Breadcrumb path="/accomplishments" />
			<ViewBlurb header="Biggest Accomplishments" author="Susie">
				You guys are obviously the coolest, but that doesn't fit on a resume (stupid page-length guidelines). I figured it'd be fun to make a list of the most awesome stuff you guys have done.
			</ViewBlurb>
		</header>
		<main class="py-2 pt-0">
            ...
            <div>
				<ul class="list-group list-group-flush" v-if="listReviews.length > 0">
					<li class="list-group-item review" v-for="accomplishment in listReviews" :class="{ 'negative-review': accomplishment.score < 2.5, 'glowing-review': accomplishment.score > 4 }">
						<div>
							<svg class="icon" v-for="icon in starIconClasses(accomplishment.score)">
								<use :href="icon"></use>
							</svg>
							<span class="fw-bold ps-2">{{ accomplishment.title }}</span>
							<small v-if="accomplishment.subject" class="ps-2 fst-italic">
								<span class="text-body-secondary">about</span>
								<span class="fw-bold ps-2">{{ accomplishment.subject }}</span>
							</small>
						</div>
						<p class="ps-4">
							<div v-if="accomplishment.text" v-html="cleanText(accomplishment.text)"></div>
							<small class="text-body-secondary" v-if="accomplishment.date">on {{ accomplishment.date }}</small>
						</p>
					</li>
				</ul>
				<div v-else class="p-2 pb-5 text-center fst-italic fs-5">
                    There don't seem to be any accomplishments for this adventurer yet.
                </div>
            </div>
		</main>
	</PageContainerVue>
...
interface ICharacterAccomplishment {
    title: string;
    /** Whose accomplishment is this? */
    subject?: string;
    date?: string;
    /** a description of what happened. */
    text: string;
    /** Score of how awesome it was on a scale of 1-5. */
    score: number;
}

interface IAccomplishmentCollection {
	all: ICharacterAccomplishment[];
	party: ICharacterAccomplishment[];
	cobb: ICharacterAccomplishment[];
	phil: ICharacterAccomplishment[];
	tero: ICharacterAccomplishment[];
}
...
export default defineComponent({
            ...
            return {
				...
				reviews: {
					...
					party: [
						...
						{
							title: "Most Omnipotent Exaltedness Max-sama, Overseer of the Nine Cosmic Planes.",
							score: 4,
							text: "Free drugs. Docked a star 'cause they weren't <em>violent</em> drugs.",
							date: "06/01/0001"
						},
						{
							title: "WaitingForeTheHamisToFall", // Hamisfore
							score: 5,
						...
						{
							title: "TrainsAreGreat", // Blackhand Roo
							score: 5,
					...
					cobb: [
						...
						{
							title: "Just A Surgeon", // Dread
							score: 4.5,
							text: "Concise.",
							date: "04/28/0001"
						},
						{
							title: "TehLeroyJenkins", // Todd Jenkins
							score: 5,
					...
					phil: [
						{
							title: "#1HandyCapableWrestlingFanboy", // Glimfolem from Battle World
							score: 0.5,
						...
						{
							title: "CEO@TankmanEnterprises.tank", // Tankman
							score: 1,
						...
						{
							title: "TheRealPurpleCentaur", // Li'l Will from Weapon World
							score: 0.5,
						...
						{
							title: "Soon-to-be Sultan", // Jafar
							score: 0.5,
							text: "Witless oaf of a streetrat.",
							date: "07/07/0001"
						},
						{
							title: "Assistant ᵗᵒ ᵗʰᵉ Great Old One", // Doyle Akor
							score: 0.5,
							text: "Guy... the loo was RIGHT THERE.",
							date: "07/07/0001"
						},
						{
							title: "GodOfTheDead", // Hades
							score: 0.5,
							text: "And I thought brimstone smelled bad. Yeesh.",
							date: "07/07/0001"
						},
						{
							title: "King of the Koopas", // Bowser
							score: 1.5,
						...
						{
							title: "CitrusGod", // Lemon Louis
							score: 0.5,
							text: "There can only be one.",
							date: "04/21/0001"
						},
						{
							title: "Ɽ⌽⾓⥌", // Somewhere Billboard spirit
							score: 4,
					...
					tero: [
						...
						{
							title: "Susie a9ebdf92-9061-4844-a7c8-e33daa4eba4c",
							score: 5,
							text: "My BFFLBPE! Best Friend Forever and Literally the Best Person Ever!",
							date: "07/07/0001"
						},
						{
							title: "Carlos@BattleWorld.gotch", // Carlos Smith, fight promoter in Battle World
							score: 4.5,
				...
				} as IAccomplishmentCollection,
				mode: "all" as keyof IAccomplishmentCollection,
				indexSelected: 0,
            }
        },
		computed: {
			...
			listReviews() {
				let selected: ICharacterAccomplishment[] = [];
				if (this.mode == "all") {
					selected = selected.concat(this.reviews.party.map(r => ({ subject: "the party as a whole", ...r})));
					selected = selected.concat(this.reviews.cobb.map(r => ({ subject: "C.O.B.B.", ...r})));
					selected = selected.concat(this.reviews.phil.map(r => ({ subject: "Li'l Phil Antonio", ...r})));
					selected = selected.concat(this.reviews.tero.map(r => ({ subject: "Tero", ...r})));
				}
				else {
					selected = this.reviews[this.mode];
				}
        ...
        methods: {
			...
			changeMenu(index: number) {
				...
				this.mode = mode as keyof IAccomplishmentCollection;
